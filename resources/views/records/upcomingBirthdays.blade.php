    ...
    <div class="col-md-8 grid-margin stretch-card">
      <div class="card">
        <div class="card-body">
          ...
          <h2 class="card-title">{{date('F')}} Celebrants </h2>
          ...
          <div class="table-responsive">
              ...
              <tbody>
                @foreach($currentMonthBirthday as $key => $currentBirthday)
               @php 
               $formattedDate = \Carbon\Carbon::parse($currentBirthday->birthday)
                ->format('F jS'); // Format as "Month Day" currentBirthday->formatted_date
                
                @endphp
                <tr>
                  <th> {{$key + 1 }}</th>
                  <td>{{ $currentBirthday->name }}</td>
                  <td>{{ $formattedDate }}</td>
                  <td>
                    <a href="{{ route('edit.record',$currentBirthday->id) }}" class="btn btn-inverse-warning" title="Edit"> <i data-feather="edit"></i> </a>
             
                    <a href="{{ route('delete.record',$currentBirthday->id) }}" class="btn btn-inverse-danger" id="delete" title="Delete"> <i data-feather="trash-2"></i>  </a>
                   </td>
                </tr>
                @endforeach
