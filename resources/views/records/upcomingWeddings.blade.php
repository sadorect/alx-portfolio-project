...
<div class="page-content">
  ...
  <nav class="page-breadcrumb">
    <ol class="breadcrumb">
      ...
      <li class="breadcrumb-item active" aria-current="page">Celebrants</li>
    </ol>
  </nav>
    ...
    <div class="col-md-8 col-sm-6 grid-margin stretch-card">
      <div class="card">
        <div class="card-body">
          ...
          <h2 class="card-title">{{date('F')}} Wedding Anniversaries</h2>
          ...
          <div class="table-responsive">
            ...
            <table id="dataTableExample" class="table table-hover dataTable no-footer" aria-describedby="dataTableExample_info">
              ...
              <tbody>
                ...
                @foreach($currentMonthWedding as $key => $currentWedding)
               @php 
               $formattedDate = \Carbon\Carbon::parse($currentWedding->wedding)
                ->format('F jS'); // Format as "Month Day" currentBirthday->formatted_date
                
                @endphp
                <tr>
                  <th> {{$key + 1 }}</th>
                  <td>{{ $currentWedding->name }}</td>
                  <td>{{ $formattedDate }}</td>
                  <td>
                    <a href="{{ route('edit.record',$currentWedding->id) }}" class="btn btn-inverse-warning" title="Edit"> <i data-feather="edit"></i> </a>
             
                    <a href="{{ route('delete.record',$currentWedding->id) }}" class="btn btn-inverse-danger" id="delete" title="Delete"> <i data-feather="trash-2"></i>  </a>
                   </td> 
                </tr>
                @endforeach
